...
public class GeneratedValueStrategy implements AnnotationStrategy {
    private static final Map<String, String> STRATEGIES = Map.of("1", "AUTO", "2", "IDENTITY", "3", "SEQUENCE", "4",
            "TABLE");
    ...
    public List<String> process(AnnotationContext context) {
        if (!context.hasFlag(AnnotationFlags.ID_PRESENT)) {
            return List.of();
        }
    ...
    private String generateAnnotation(Scanner scanner) {
        String strategy = PromptUtils.chooseOption(scanner, "Choose strategy:", STRATEGIES);
